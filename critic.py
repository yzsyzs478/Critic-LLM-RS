...
def parse_args():
    ...
    parser.add_argument("--final_model_path", type=str, default="final_model.pth")
...
def process_data(data,
                 ...
                 embedding_size: int,
                 max_seq_length: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for X_val_batch, y_val_batch in val_loader:
                ...
                val_labels.extend(y_val_batch.cpu().numpy().tolist())
        ...
        if curr_score > best_val_score:
            ...
            print(f"Best fune-tuning-model updated at epoch {epoch + 1} with Val micro-P: {micro_p:.4f}")

    torch.save(model.state_dict(), final_model_path)
    print(f"Final fune-tuning-model saved to {final_model_path}")
...
def test_model(
    ...
    if len(test_labels):
        micro_p, micro_r = compute_micro_pr(
            test_labels, test_preds, num_classes=num_classes
        )
    micro_f1 = compute_micro_f1(micro_p, micro_r)

    print(f"{label} - Test micro-P: {micro_p:.4f}, "
