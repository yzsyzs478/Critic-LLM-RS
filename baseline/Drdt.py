...
def divergent_thinking_preferences(
    ...
    prefs = call_llm_json(messages, temperature=temperature, max_attempts=2)
    return prefs
...
def reflect_preferences_with_feedback(
    ...
    updated = call_llm_json(messages, temperature=temperature, max_attempts=2)
    if updated:
        return updated
...
def processBatch_drdt_style(
    ...
    for idx, user in enumerate(users, start=1):
        ...
        pref_profile = divergent_thinking_preferences(initial_prefix, initial_cic_ex, temperature=0.3)
        if not pref_profile:
            pref_profile = {}
        step_profiles = []
...
if __name__ == "__main__":
    ...
    output_file = "llm_movie_drdt.json"
