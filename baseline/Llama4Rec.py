...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Preference-aware LLM movie recommendation (no Critic)."
    )
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="llm_movie_llamarec.json",
...
def suggestMovie(
    ...
    for movie in watchedMoviesSubset:
        prompt += (
            f"User watched: {movie.get('title', '')} directed by {movie.get('directedBy', '')} "
            f"starring {movie.get('starring', '')} rating {movie.get('rating', '')}. "
        )
...
def processBatch(
    ...
    for user in users:
        user_results = {"user_id": user.get("user_id")}
