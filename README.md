...
- Calls the LLM to generate **recommendations** for each user.
...
All evaluation scripts are located in the `metric/` directory and support two evaluation settings. Currently, the LLM in critic_llm_rs.py generates recommendations directly, so Option A is used for evaluation. If the LLM is instead modified to select recommendations from a predefined candidate item set, the corresponding evaluation should use Option B.


#### Option A: Critic + Real  
(*Critic Rating + Real Rating for LLM-generated recommendations*)

1. **Extract & clean recommended titles:**
   ```bash
   python metric/extract_filter.py --input outputs/llm_critic_movie.jsonl
   ```
