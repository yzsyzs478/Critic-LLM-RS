...
- `llm_rs.py`  
  A basic LLM-only recommendation pipeline (without critic feedback), which can serve as a simple baseline.
...
This subset is used to train the **Recommendation Critic** model.

- Contains **30,000 users**, each with at least **10 item interactions**.
- For each user:
  - **One item** is randomly selected as the **target item** to be predicted.
  - The **remaining items** are treated as the user’s **historical interactions**.
- The Recommendation Critic learns to predict the rating of the target item **conditioned on** the user’s interaction history, thereby modeling personalized preference.
...
(Other common dependencies such as `numpy`, `pandas`, etc., may be required depending on your setup.)
...
First, deploy an LLM locally **or** configure the corresponding API endpoint. Then run:
