...
First, deploy an LLM locally **or** configure the corresponding API endpoint. Configure the API in `critic-llm-rs.py`

When calling the API, configure the following arguments in `critic-llm-rs.py`:
...
```
By replacing the values of api_key, base_url, and llm_model_name above, you can connect to the corresponding LLM service and invoke the model to generate recommendations.
Then run:
